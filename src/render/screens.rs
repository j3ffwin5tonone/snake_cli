...
pub fn draw_start_menu(
    ...
    centered_text("SNAKE", 80.0, 56.0, Color::from_rgba(120, 230, 120, 255));

    let panel_w = 460.0;
    let panel_h = 320.0;
    let panel_x = WINDOW_WIDTH as f32 / 2.0 - panel_w / 2.0;
    draw_panel(panel_x, 130.0, panel_w, panel_h);
    ...
    centered_text(name_label, 165.0, 18.0, LIGHTGRAY);
    ...
    centered_text(
        &format!("Board: {}  [B] toggle", preset.label()),
        268.0,
        22.0,
        WHITE,
    );
    ...
    if mode == GameMode::Daily {
        centered_text(
            &format!("Today's challenge · Seed: {}", Game::daily_date_key()),
            320.0,
            18.0,
    ...
    if achievements.unlocked_count() > 0 {
        let sample = ACHIEVEMENTS
            .iter()
            .find(|a| achievements.is_unlocked(a.id));
        if let Some(ach) = sample {
            centered_text(
                &format!("Latest: {} — {}", ach.title, ach.description),
                392.0,
                16.0,
                Color::from_rgba(120, 230, 120, 200),
            );
        }
    }
    centered_text("[Enter] Start", 400.0, 24.0, WHITE);
    centered_text("[M] Mode  [B] Board  [T] Sound", 430.0, 16.0, LIGHTGRAY);
    centered_text("[P] Pause in game   [Q] Quit", 452.0, 16.0, LIGHTGRAY);
}
